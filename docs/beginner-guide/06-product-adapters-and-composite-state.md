...
当前 exact identity 是 `0.3.0` / revision 5 /
`a07c5723568cfcbf379a2173225122dc5db4e2168a50700d7f256aba3de5957e`。adapter ID 固定为
`media-backup-current-0.3.0-r1`，manifest 的唯一 current version 固定为 3；仅接受 version 3。
...
当前 exact identity 是 `0.9.26` / revision 7 /
`5c4a32f3f1813e6e6ef528b55e25e912bfe0191f79332ad5538943746c8f17a3`。Host 使用
...
记录 `kind=credentials-key`、key ID、key SHA-256、`algorithm=sarmg-secret-envelope-aes-256-gcm`、`envelope_version=1`；记录摘要
不等于拥有 key。
...
“ID 相同但 bytes 不同”的 key 被误接受。Foundation 密封格式按产品域派生密钥，设备授权码绑定 device ID，
operation request 绑定 operation ID、action 与 `request_ciphertext` 字段域；空绑定或认证失败均拒绝。
...
SQLite + MediaMTX config/contract + recordings tree + external key 是一个组合。当前 `0.2.2` / revision 7 /
`bb64805d1434fa953b5a215c636c086d98bce467825f7e9b6d3a5c1c0bd359c4` adapter 使用 `backup-current`、
`verify-current`、`restore-current` 和 `recover-current`，严格要求 `mediamtx.lock`、`mediamtx.yml`、
`sentinel.env` 三个配置、recordings 树与当前 credentials key。工具认证 `sentinel_clients.authorization_code_enc`，并核对授权码摘要。
任何缺失、额外、改名或内容不一致都会使完整 verify 失败；不能使用 generic SQLite 冒充完整备份。

## 6.7 Dufs

SQLite + protected YAML + shared root 构成状态。当前 `0.51.0` / revision 1 /
`3659ff0c703515f555af95f0f1c08c35fa0555a8978f5f0e5a658fd93d225423` adapter 使用同一组 `*-current` 命令，
并要求配置集恰好是 `dufs.yaml`。备份校验数据库中记录的 shared root 设备号/inode；恢复时重绑定到新目录，并在 journal 中保存源与目标数据库摘要。
