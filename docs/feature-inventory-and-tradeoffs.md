...
| UPG-093 | CI 分离普通质量与 source-bound release | `.github/workflows/ci.yml`、`release.yml` | 开发运维 | 高 | release 可能绕过测试或在 publish job 执行源码 | fmt/check/clippy/test/supply-chain；publish job 仅从事件提交读取发行说明，签名和打包只用构建制品；artifact provenance |
